...
def compare_prints2(path_a, path_b, thresh=110, debug=False):
    m_a = find_minutae(path_a, disp=debug)
    m_b = find_minutae(path_b, disp=debug)
    if m_a.size == 0 or m_b.size == 0:
        #print("No minutiae found in one or both images.")
        return 0
    
    c_a = np.expand_dims(centroidnp(m_a), 0)
    c_b = np.expand_dims(centroidnp(m_b), 0)

    # Ensure m_a and m_b are (N, 2) arrays for consistent operations
    dists_a = np.linalg.norm(m_a - c_a, axis=1)
    dists_b = np.linalg.norm(m_b - c_b, axis=1)

    try:
        sort_dists = np.array(sorted(dists_a)) - np.array(sorted(dists_b))
    except Exception as e:
        #print(f"Error in sorting or subtracting distances: {e}")
        return 0
...
def compare_prints3(path_a, path_b):
	imA = Image.open(path_a)
	imB = Image.open(path_b)
...
def main(directory_1, directory_2):
    ...
    for directory in [directory_1, directory_2]:
        ...
        for f in f_files:
            ...
            simscoreref2 = compare_prints2(f_path, reference_f_path)
            simscoreref3 = compare_prints3(f_path, reference_f_path)
            falseposvotes = 0
            ...
            simscorecor2 = compare_prints2(f_path, s_path)
            simscorecor3 = compare_prints3(f_path, s_path)
            trueposvotes = 0
